        ...
        for d in range(nD):
            ...
            if d == 0:
                if nE[d] != nFlow_out[d] + nLost[d] + nCons[d]:
                    print(
                        "Error count out (Day %d): %d != %d + %d + %d"
                        % (d, nE[d], nFlow_out[d], nLost[d], nCons[d])
                    )
                else:
            ...
            # last day
                ...
                if nE[d] != nFlow_in[d] + nGain[d] + nCons[d - 1]:
                    print(
                        "Error count in (Day %d): %d != %d + %d + %d"
                        % (d, nE[d], nFlow_in[d], nGain[d], nCons[d - 1])
                    )
                else:
...
def plot_flows(nE, nFlow_out, nFlow_in, nCons, nLost, nGain):
    # ------------------------------------------
    # figsize measures
    a = 4.0  # height of the main day
    b = 5.0  # inter-day spacing
    m = 0.5  # margin
    ...
    for d in range(nD):
        ...
        axe.add_patch(p)
        l = lines.Line2D(
            [x + lx, x + lx + z / 3.0, x + lx], [0, z / 2.0, z], lw=2.0, color="w"
        )
        axe.add_line(l)
        # --flow_in
        z = nFlow_in[d] / norm
        ...
        if d < nD - 1:
            ...
            path_data = [
                ...
                (Path.CURVE4, (x + bx / 2.0, zo + zc)),
                (Path.CURVE4, (x + bx / 2.0, zi + zc)),
                (Path.CURVE4, (x + bx, zi + zc)),
                (Path.LINETO, (x + bx, zi)),
                (Path.CURVE4, (x + bx / 2.0, zi)),
                (Path.CURVE4, (x + bx / 2.0, zo)),
                (Path.CURVE4, (x + lx, zo)),
            ...
            axe.add_patch(p)
        # --day
        p = patches.Rectangle((x, 0), lx, ze, facecolor=day_col, edgecolor="none")
        axe.add_patch(p)
        axe.text(
            x + lx / 2.0,
            -dr / 2.0,
...
def plot_evol(lGraph, lEdges, lC):
    ...
    for d in range(nD - 1):
        ...
        # ------------------------------------------
        ax_loc = fig2.add_axes([d / (nD - 1.0) + mx, 0.15, w, 0.79])
        ax_loc.loglog(
            x[1:],
            hist[0],
            "o",
            mec=List_Col[d],
            mfc="none",
            label=str(d + 1) + r"$\rightarrow$" + str(d + 2),
        )
...
def flow_probability(lGraph, lFlow_out, lFlow_in, lCons, lLost, lGain):
    ...
    for d in range(nD - 1):
        ax = fig.add_axes([d / (nD - 1.0) + mx, 0.15, w, 0.79])
        #        b = np.zeros(nX)
    ...
    for d in range(1, nD):
        ax = fig.add_axes([(d - 1) / (nD - 1.0) + mx, 0.15, w, 0.79])
        ax.stackplot(
            lX[d],
            lCFi[d] / lN[d],
            lCC2[d] / lN[d],
            lCG[d] / lN[d],
            colors=[fi_col, c_col, g_col],
        )
        ax.set_xscale("log")
        ax.set_xlim((lX[d][0], lX[d][-1]))
        ax.set_xlabel("$w$", fontsize=fsize)
